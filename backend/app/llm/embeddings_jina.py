"""Embeddings vía Jina AI (endpoint OpenAI-compatible).

Modelo: jina-embeddings-v3 → 1024 dims (padded a 1536).
API key: https://jina.ai/embeddings/
Free tier: 10M tokens únicos.
Límite: 8.192 tokens por texto individual.
"""
...
# Jina acepta hasta 2048 textos/batch pero el límite real es de tokens.
# Configuramos conservador: 50 textos o 6.000 tokens por batch.
_BATCH_CONFIG = BatchConfig(
    max_texts_per_batch=50,
    max_tokens_per_batch=6_000,
    max_tokens_per_text=6_000,  # Jina acepta hasta 8.192, con margen.
)
...
class JinaEmbeddingsProvider:
    ...
    def __init__(self) -> None:
        ...
        if not key:
            raise RuntimeError("JINA_API_KEY no configurada")
        ...
        self._model = settings.jina_model or "jina-embeddings-v3"
